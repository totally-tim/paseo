...
A Paseo-owned worktree is never a project root. Any path inside the daemon's worktrees root resolves to the main checkout's project, so a bare `paseo run` from inside a worktree no longer allocates a second project named after the worktree slug. Hand-made linked worktrees are not covered and can be their own project. Workspace reuse is keyed on the exact cwd, never an enclosing or enclosed directory, so a worktree workspace whose cwd is a subproject does not answer for the worktree root and vice versa: an agent create with no workspace id lands in the active workspace at that cwd when one exists, and a `workspace.create` with a directory source inside a Paseo-owned worktree returns the workspace with that cwd. A `workspace.create` on an ordinary directory keeps minting a fresh record, so two workspaces may share one cwd. Scheduled runs and Hub creates always mint their own record because they archive it when the run finishes.
