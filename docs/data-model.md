...
A Paseo-owned worktree is never a project root. While git still recognizes the worktree, any path inside the daemon's worktrees root resolves to the main checkout's project, so a bare `paseo run` from inside a worktree no longer allocates a second project named after the worktree slug. Hand-made linked worktrees are not covered and can be their own project. Workspace reuse is keyed on the exact cwd, never an enclosing or enclosed directory, so a worktree workspace whose cwd is a subproject does not answer for the worktree root and vice versa. Inside a Paseo-owned worktree, an agent create with no workspace id and a `workspace.create` with a directory source both land in the workspace at that cwd, restoring it when it is archived and its project is not. Ordinary directories always mint a fresh record: two workspaces may share one cwd, and scheduled runs and Hub creates mint throwaway records that they archive, with every agent inside, when the run finishes, so a bare create must never join one.
