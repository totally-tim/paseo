...
Account wire fields are optional and the client gates account management with `server_info.features.providerAccounts`. Continuation and the host queue use `server_info.features.agentContinuation`; older hosts show an update requirement. Metadata and catalogs require `daemon.read`; login links, one-time codes, and mutations require `daemon.manage`. Queue inspection uses `workspace.read`; queue changes and recovery cancellation use `workspace.write`. Login requests are not queued for replay. Never include login challenges or provider output in logs, handoff exports, profiles, or persisted client state.
