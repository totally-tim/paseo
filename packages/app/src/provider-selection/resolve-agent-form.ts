...
function sameInitialValues(left: FormInitialValues = {}, right: FormInitialValues = {}): boolean {
  return (
    ...
    left.accountSelection === right.accountSelection &&
    left.continuationPolicy === right.continuationPolicy
  );
}
...
function receiveInputs(
  ...
  const sameHost = previous?.serverId === action.serverId;
  let next = state;
  if (changed) {
    const resolved = resolveAgentForm(state, {
      type: action.isVisible ? "REQUEST_RESOLUTION" : "RESET",
    });
    next = {
      ...resolved,
      ...
      form: {
        ...resolved.form,
        // A host switch or a new explicit selection invalidates the current
        // account and continuation choices; otherwise keep what the user picked.
        accountSelection:
          initial?.accountSelection ?? (sameHost ? state.form.accountSelection : undefined),
        continuationPolicy:
          initial?.continuationPolicy ?? (sameHost ? state.form.continuationPolicy : undefined),
      },
    };
  }
