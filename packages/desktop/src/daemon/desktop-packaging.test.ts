...
function createFakeMacBundle(options: { includeHelper: boolean }): {
  ...
  const appPath = join(root, "Forkeo.app");
  const contentsPath = join(appPath, "Contents");
  const resourcesPath = join(contentsPath, "Resources");
  const shimPath = join(resourcesPath, "bin", "paseo");
  const mainPath = join(contentsPath, "MacOS", "Forkeo");
...
describe("desktop packaging", () => {
  ...
  it("registers Forkeo agent links with the operating system", () => {
    const config = readFileSync(join(packageRoot, "electron-builder.yml"), "utf8");

    expect(config).toContain("name: Forkeo agent link");
  ...
  it("fails packaged macOS CLI startup when Helper is missing", () => {
    ...
    try {
      ...
      expect(result.stderr).toContain("Bundled Forkeo Helper executable not found");
      expect(result.stdout).not.toContain("main-executable");
    } finally {
      rmSync(bundle.root, { recursive: true, force: true });
    }
