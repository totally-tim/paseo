...
| `respondToPermission(requestId, response)` | `PaseoAgentPermissionResolved`    | Answers a pending permission with an allow or deny `response`. Resolves with the daemon's resolution and rejects when the request is already gone. |
